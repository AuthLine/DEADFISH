...
const epibot_websocket = require ('./websocket.base');

class epibot_websocket_client_deribit extends epibot_websocket.client {
  constructor (conf = {}) {
    super (conf);
  }
...
module.exports = class epibot_websocket_deribit  extends epibot_websocket.base {

  constructor (stub, account) {
    super (stub, account);
    this.ticker = {};
    this.ws = new epibot_websocket_client_deribit ({
      exchange: 'deribit',
      stub: stub,
      apikey: this.apikey,
      secret: this.secret,
  ...
  parse_channel_trades (message) {
    ...
    if (message.params.channel.indexOf ('trades.') == 0) {
      for (var i = 0; i < message.params.data.length; i++) {
        ...
        const exchange = message.epibot.exchange;
        const stub = message.epibot.stub;
