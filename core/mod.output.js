...
module.exports = class epibot_output_module extends epibot_module {
    ...
    initialize(mode = 'normal') {
        ...
        this.logfile = base_dir + '/log/epibot.log'
        this.reset();
        this.load_language();
    }
    ...
    */
    ...
    outws(uuid, type, message) {
        if (global.hasOwnProperty('epibot'))
            if (global.epibot.hasOwnProperty('wss'))
                    global.epibot.wss.emit('proxy', logentry)
    }
    ...
    add_message(type, message, settings = {}) {
        ...
        if (!['section', 'subsection','blank'].includes(type)) {
            ...
            if (global.hasOwnProperty('epibot'))
                if (global.epibot.hasOwnProperty('wss'))
                    global.epibot.wss.emit('log', logentry)
        }
    ...
    // Parse raw output into a epibot_output object
